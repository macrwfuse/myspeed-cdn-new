 ...
 * 3. CDN 节点探活扫描
 *
 * ⚠️ 本模块位于 server/util 而非 scripts/，因为编译后的二进制(如 Windows 的
 *    MySpeed-CN.exe)只打包 server/index.js 的依赖树，访问不到 scripts/ 目录。
 *    运行时更新(server/util/cdnUpdater.js)必须能 import 到它。
 *    scripts/cdn-discovery.mjs 现在只是本文件的转发层 + CLI 入口，供容器调度器
 *    与手工执行使用，逻辑保持单一来源。
 */
 ...
 * 判断 URL 属于哪个 CDN 组。
 *
 * 取「最长匹配域名」而非「首个匹配的组」：域名列表里有互相包含的条目
 * (如 'qq.com' 与 'webcdn.m.qq.com')，按列表顺序返回会把腾讯云节点错判成
 * 360 云组，从而在替换时换成别家的 CDN —— 那会改变测速目标的归属。
 */
export function matchCdnGroup(url) {
  let group = null;
  let longest = 0;

  for (const [name, domains] of Object.entries(CDN_DOMAIN_GROUPS)) {
    for (const d of domains) {
      if (d.length > longest && url.includes(d)) {
        group = name;
        longest = d.length;
      }
    }
...
/**
 * 已知稳定 CDN 下载源(备用池种子)。
 * 各大厂长期有效的下载端点，作为发现结果的兜底 —— 自动发现拿不到新链接时
 * 仍然有可用的替换源。
 */
export const KNOWN_CDN_SOURCES = {
  'Speedo云CDN': [
    'https://lf9-apk.ugapk.cn/package/apk/aweme/5072_340301/aweme_douyin-huidu-gw-aweme-3430_v5072_340301_eea8_1747058635.apk',
    'https://cdn.aixifan.com/downloads/AcfunLive-Setup-1.9.0.200-ReleaseX64_6d5c40.exe',
    'https://devtools.qiniu.com/linux/amd64/qrsctl',
